...
family_rules = [
    ...
    IF(AND(OR('brother (?x) (?y)', 'sister (?x) (?y)'), 'parent (?x) (?z)', 'parent (?y) (?w)'), NOT('same-identity (?x) (?y)'), THEN('cousin (?z) (?w)')),
    IF(AND('parent (?x) (?y)', 'parent (?y) (?z)'), THEN('grandparent (?x) (?z)')),
    IF('grandparent (?x) (?y)', THEN('grandchild (?y) (?x)'))]                    # fill me in
